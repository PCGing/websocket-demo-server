...
public class DataTablePageUtil {
	...
	public DataTablePageUtil(HttpServletRequest request) {
		...
		String orderColumn2 = request.getParameter("Order[1][column]");
		orderColumn2 = request.getParameter("columns[" + orderColumn2 + "][data]");
		String orderColumn3 = request.getParameter("Order[2][column]");
		orderColumn3 = request.getParameter("columns[" + orderColumn3 + "][data]");
		// 获取排序方式
		String orderDir = request.getParameter("Order[0][dir]");
		if (orderDir == null) {
			orderDir = "asc";
		}
		String orderDir2 = request.getParameter("Order[1][dir]");
		if (orderDir2 == null) {
			orderDir2 = "asc";
		}
		String orderDir3 = request.getParameter("Order[2][dir]");
		if (orderDir3 == null) {
			orderDir3 = "asc";
		}
		String orderStr = (orderColumn == null ? "" : (" Order by " + orderColumn + " " + orderDir + " "))
				+ (orderColumn2 == null ? "" : "," + (orderColumn2 + " " + orderDir2 + " "))
				+ (orderColumn3 == null ? "" : "," + (orderColumn3 + " " + orderDir3 + " "));
	...
	public String getSearchStr() {
		if (!getSearch().equals("")) {
			String str_search_str = "1 = 2 ";
			String[] array_str_search = getColumns_name().split(",");
			for (int i = 0; i < array_str_search.length; i++) {
				str_search_str += "or " + array_str_search[i] + " like '%" + getSearch() + "%' ";
			}
			setSearchStr(" and (" + str_search_str + ")");
		} else {
			setSearchStr("");
		}
		return searchStr;
